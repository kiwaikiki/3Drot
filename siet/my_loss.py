...
class GSLossCalculator:
    ...
    @staticmethod
    def calculate_angle_loss(pred_z, pred_y, true_transform):
        z = pred_z
    ...
    def get_val_angle_loss(self, pred_z, pred_y, true_transform):
        self.val_losses.append(self.calculate_angle_loss_GS(pred_z, pred_y, true_transform).detach().item())

    def calculate_train_loss(self, pred_z, pred_y, transform): 
    ...
    def calculate_val_loss(self, pred_z, pred_y, transform): 
        loss_z = torch.mean(get_angles(pred_z, transform[:, :3, 2].cuda()))
